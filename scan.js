...
/* =====================================================
   ✅ UPGRADED OCR — Google MediaPipe ML Kit (Web)

   THE REAL FIX:
   ChatGPT wala "google.search.tasks.vision" FAKE tha —
   koi aisi cheez exist nahi karti. Isse sirf ReferenceError
   milta.

   Sahi tarika: Dynamic ESM import directly from CDN.
   FilesetResolver + TextRecognizer tab hi load hote hain
   jab OCR actually call ho — koi global dependency nahi,
   koi script tag nahi, koi crash nahi.
   ===================================================== */
...
async function runOCR(file) {
    ...
    try {
        /* ✅ CORRECT: Dynamic import from official MediaPipe ESM bundle
           — Seedha CDN se aata hai runtime pe
           — Koi <script> tag zaroorat nahi HTML mein
           — Koi global ReferenceError possible nahi */
        const { FilesetResolver, TextRecognizer } = await import(
            "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/vision_bundle.mjs"
        );

        /* Step 1: WASM resolver initialize karo */
        const filesetResolver = await FilesetResolver.forVisionTasks(
            "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm"
        );

        /* Step 2: TextRecognizer banao */
        ...
        /* Step 3: File ko ImageBitmap mein convert karo */
        const imageBitmap = await createImageBitmap(file);

        /* Step 4: Text recognize karo */
        const result = await textRecognizer.recognize(imageBitmap);

        /* Step 5: ✅ Kachra Sanitizer — OCR garbage saaf karo */
        ...
        if (cleaned.length === 0) {
            resultBox.innerText = "No clear text found. Try again.";
            return;
        }
        ...
        resultBox.innerText = "OCR Error. Check connection & try again.";
    }
}
...
/* =====================================================
   ✅ UPGRADED TRANSLATOR — Google GTX Mirror
   Replaces: MyMemory (500-char limit)
   New: Unlimited characters, full paragraph support
   ===================================================== */
...
if (translateBtn) {
    translateBtn.onclick = async function () {
        ...
        if (!text || text === "Waiting for scan..." || text === "Eris AI is reading...") {
            alert("Bhai, pehle kuch scan toh karle!");
            return;
        }
        ...
        resultBox.innerText = "Eris AI Translating...";
        document.querySelector(".result-floating").style.display = "block";
