...
/* =====================================================
   ...
   HOW IT WORKS:
   1. Image → Canvas (Grayscale + Contrast + Brightness)
   2. Pre-processed canvas → Tesseract v5
   3. Result clean karo → show karo

   WHY CANVAS PRE-PROCESSING:
   Raw image pe Tesseract ~60% accurate hota hai.
   Grayscale + Contrast filter ke baad ~90%+ ho jata hai.
   Yahi Google Lens ka basic secret hai.

   NOTE: index.html mein ye script daalni hai <head> mein:
...
async function runOCR(file) {
    ...
    try {
        /* Step 1: Image → ImageBitmap → Canvas */
        ...
        /* Step 2: Google-Level Filter — Grayscale + Contrast + Brightness */
        ctx.filter = "grayscale(100%) contrast(160%) brightness(110%)";
        ctx.drawImage(image, 0, 0, canvas.width, canvas.height);

        /* Step 3: Tesseract v5 — Hindi + English */
...
if (translateBtn) {
    translateBtn.onclick = async function () {
        ...
        /* Basic Checks */
        if (!text || text.includes("Waiting") || text.includes("reading") || text.includes("analyzing")) {
...
console.log("Eris Scan — Tesseract v5 OCR + Canvas Pre-Processing + Google Translate Loaded ✅");
